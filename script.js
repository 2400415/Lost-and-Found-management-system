    ...
    if(itm) return `<div style="margin:6px 0"><b>${itm.name}</b> — ${itm.type.toUpperCase()} (${itm.category}) reported by ${itm.reporter}</div>`;
    ...
    saveNotifications();
    renderNotifyBadge();
    alert(`🔔 Possible match detected! ${matches.length} potential match(es). Check the Notifications tab.`);
}
}
    ...
    if(!u){ alert('Invalid credentials — try sample accounts or register.'); return; }
    showAppFor(u);
}
